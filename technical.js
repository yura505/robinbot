...
ADX: function(x) {
    ...
    for (let i = x.length-2; i >= 0; i--) {
        ...
        let DirectionalMovementPlus = x[i].high-x[i+1].high > x[i+1].low-x[i].low ? Math.max(x[i].high-x[i+1].high, 0): 0;
        let DirectionalMovementMinus = x[i+1].low-x[i].low > x[i].high-x[i+1].high ? Math.max(x[i+1].low-x[i].low, 0): 0;
        x[i]['SmoothedTrueRange'] = (x[i+1]['SmoothedTrueRange'] == undefined) ? TrueRange : 
            x[i+1]['SmoothedTrueRange'] - x[i+1]['SmoothedTrueRange']/length + TrueRange;
        x[i]['SmoothedDirectionalMovementPlus'] = (x[i+1]['SmoothedDirectionalMovementPlus'] == undefined) ? DirectionalMovementPlus :
            x[i+1]['SmoothedDirectionalMovementPlus'] - x[i+1]['SmoothedDirectionalMovementPlus']/length + DirectionalMovementPlus;
        x[i]['SmoothedDirectionalMovementMinus'] = (x[i+1]['SmoothedDirectionalMovementMinus'] == undefined) ? DirectionalMovementMinus :
            x[i+1]['SmoothedDirectionalMovementMinus'] - x[i+1]['SmoothedDirectionalMovementMinus']/length + DirectionalMovementMinus;
        x[i]['DIPlus'] = x[i]['SmoothedDirectionalMovementPlus'] / x[i]['SmoothedTrueRange'] * 100;
        x[i]['DIMinus'] = x[i]['SmoothedDirectionalMovementMinus'] / x[i]['SmoothedTrueRange'] * 100;
    }
    if (x.length > length) {
...
ATR: function(x, length) {
    for (let i = x.length-length-2; i >= 0; i--) {
        ...
        x[i]['ATR'] = (prev_atr * (length-1) + this.TR(x[i+1], x[i])) / length
    }
    return x[0]['ATR'];
},
...
SAR: function(x) {
    ...
    for (let i = x.length-3; i >= 0; i--) {
        if (x[i+1].psar === undefined)
            x[i+1].psar = x[i+1].close;
        if (bull)
            x[i].psar = x[i+1].psar + af * (hp - x[i+1].psar)
        else
            x[i].psar = x[i+1].psar + af * (lp - x[i+1].psar)
        var reverse = false;
        if (bull) {
            if (x[i].low < x[i].psar) {
                bull = false;
        ...
        if (!reverse) {
            if (bull) {
                ...
                if (x[i+1].low < x[i].psar)
                    x[i].psar = x[i+1].low;
                if (x[i+2].low < x[i].psar)
                    x[i].psar = x[i+2].low;
            } else {
                if (x[i].low < lp) {
                    lp = x[i].low;
                ...
                if (x[i+1].high > x[i].psar)
                    x[i].psar = x[i+1].high;
                if (x[i+2].high > x[i].psar)
                    x[i].psar = x[i+2].high;
            }
        }
    }
