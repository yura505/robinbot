...
module.exports = {
  ...
  soon: function(symbol) {
    ...
    if (e !== undefined) {
      return (
        (e.date == dates.today && e.timing == "pm") ||
        (e.date == markets.nextDate && e.timing == "am")
      );
    }
  }
};
...
function download_earnings(cb) {
  if (global.Robinhood !== undefined && conf.earnings_announcements) {
    ...
    global.Robinhood.earnings({ range: days }, function(err, resp, body) {
...
function download_recent_earnings(cb) {
  ...
  request(url, function(err, resp, body) {
    ...
    if (_list.length > 0) return download_recent_earnings(cb);
    cb();
  });
}
