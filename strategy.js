...
var strategy = (module.exports = {
  ...
  run(list) {
    ...
    list.forEach(symbol => {
      ...
      log +=
        pad(7, n(TA.close).format("0.00"))[
          TA.percentage > 0 ? "green" : "red"
        ] + " ";
      log +=
        pad(6, n(TA.sma50).format("0.0"))[
          TA.sma50 < TA.sma200 ? "red" : "reset"
        ] + " ";
      log +=
        pad(6, n(TA.sma200).format("0.0"))[
          TA.sma50 < TA.sma200 ? "red" : "reset"
        ] + " ";
      log +=
        pad(6, n(TA.ss143.K).format("0.0"))[
          TA.ss143.K > TA.ss143.D ? "green" : "red"
        ] + " ";
      log +=
        pad(3, n(TA.adx).format("0"))[TA.adx > 20 ? "yellow" : "reset"] + " ";
      log +=
        pad(7, n(TA.macdh0).format("0.00"))[TA.macdh0 > 0 ? "green" : "red"] +
        " ";
      log +=
        pad(6, n(TA.macdh).format("0.00"))[TA.machd > 0 ? "green" : "red"][
          (TA.macd0 < 0 && TA.macd > 0) || (TA.macd0 > 0 && TA.macd < 0)
            ? "inverse"
            : "reset"
        ] + " ";
      log +=
        pad(
          9,
          n(TA.sar.psar).format("0.00") +
            (TA.sar.bull ? "^" : "v") +
            (TA.sar.reverse ? "R" : " ")
        )[TA.sar.bull ? "green" : "red"][TA.sar.reverse ? "inverse" : "reset"] +
        " ";
      log += pad(7, n(TA.atr).format("0.00")) + " ";
      log +=
        pad(6, n(TA.obv / TA.obv_ma5).format("0.000"))[
          TA.obv > TA.obv_ma5 ? "green" : "red"
        ] + " ";
      log +=
        pad(5, n(TA.rsi).format("0.00"))[TA.rsi > 50 ? "green" : "red"] + " ";
  ...
  k39(symbol, TA) {
    ...
    if (
      ...
      signal = "BUY";
    } else if (TA.ss391.K < 50 && TA.obv < TA.obv_ma30) {
      signal = "SELL";
    }
    return signal;
  },
  ...
  slow_stoch(symbol, TA) {
    ...
    let ssd = (x => {
      ...
      for (let i = 20; i >= 0; i--) {
        ...
        if (ss.K < 20) status = "oversold";
        else if (ss.K > 80) status = "overbought";
        else if (ss.K > 50 && status == "BUY") status = null;
        else if (ss.K < 50 && status == "SELL") status = null;
        else if (ss.K > 20 && status == "oversold") status = "BUY";
        else if (ss.K < 80 && status == "overbought") status = "SELL";
      }
      return status;
    })(quotes.get(symbol));
